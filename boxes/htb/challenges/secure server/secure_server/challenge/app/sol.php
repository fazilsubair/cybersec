<?php
    ...
    if (isset($_GET["project"])) {
      ...
      $project = str_replace("../", "", $project);  // Remove any instances of "../"
      $project = strtolower($project);             // Convert to lowercase
    }
?>
         ...
         <div id="container">
            ...
            <div class="inner rpgui-container framed" style="position:relative">
               ...
               <?php foreach ($allProjects as $proj): ?>
                  <option value="<?= htmlspecialchars($proj, ENT_QUOTES, 'UTF-8') ?>" <?= $proj == $project ? "selected" : "" ?>>
                        <?= htmlspecialchars(ucfirst($proj), ENT_QUOTES, 'UTF-8') ?>
                  </option>
               <?php endforeach; ?>
               </select>

               <div id="containers" style="margin-top:30px;" class="rpgui-container framed-grey">
                  <h1><?php echo htmlspecialchars(ucfirst($project), ENT_QUOTES, 'UTF-8'); ?></h1>
                  <p>
                     <?php
                           // Ensure the file path is safe and valid
                           $projectPath = basename($project);  // Strip out any directory paths
                           $projectFile = "../projects/" . $projectPath;
