...
if Request_Form("Username") = tcWebApi_get("Account_Entry0","username","h") then
	if tcWebApi_get("Sys_Entry","IsAdmin","h") <> "0" then
		if tcWebApi_get("Account_Entry0","Logged","h") = "1" then%>
			document.write('<% tcWebApi_get("Account_Entry0","username","s") %>' + ' user has logged in!');
			Logged = 1;
		<%elseif tcWebApi_get("Account_Entry1","Logged","h") = "1" then%>
			document.write('<% tcWebApi_get("Account_Entry1","username","s") %>' + 'user has logged in!');
			Logged = 1;
...
function btnbandReg()
{
	...
	{
		alert("Incorrect connection to ITMS, unable to register device!");
	}
...
if (LoginTimes >= 3)
{
	document.write('You have already three consecutive failed to login, wait one minute before login!');
}
</script>
<DIV id=div_visite>
...
<TABLE borderColor="#e7e7e7" height="45%" cellSpacing="0" cellPadding="0" align="center" bgColor="#e7e7e7" border="0">
  ...
  <TR>
    <TD align="middle" bgColor="#ffffff" colSpan="3" height="10"><LABEL><IMG height="117" src="/img/logo.jpg" width="337"></LABEL></TD></TR>
        <TR> 
          <TD align=right width="30%" height=50>Administrator account:</TD>
    <TD width=30>　</TD>
    <TD><LABEL>
  ...
  <TR>
    ...
    <TD style="COLOR: #ff0000" align="center" colspan="3">
      <SCRIPT language=javascript>    
			...
			{
				var str = 'You have already entered ' +  LoginTimes + 'times wrong user name or password!';
				document.write(str);
			}			
      </SCRIPT>
    </TD></TR>
  <TR>
	...
	<% else %>
	<INPUT id="bandReg" onclick="btnbandReg()" type="button" value="Register" name="bandReg">&nbsp; 
	<% end if %> 
