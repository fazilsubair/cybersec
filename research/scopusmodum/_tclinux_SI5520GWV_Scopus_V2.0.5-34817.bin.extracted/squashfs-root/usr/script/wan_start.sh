...
if [ "$TCSUPPORT_NAT_LOOPBACK" != "" ]; then
	VS_LOOPBACK_CHAIN=VS_LOOPBACK_$WAN_IF
fi
...
customized_mac(){
	echo $ServiceList | grep "INTERNET"
	if [ $? = 0 ];then
		/sbin/ifconfig $NASName hw ether $INTERNET_MAC
	else
		echo $ServiceList | grep "OTHER"
		if [ $? = 0 ];then
			/sbin/ifconfig $NASName hw ether $IPTV_MAC
		else
			echo $ServiceList | grep "VOICE"
			if [ $? = 0 ];then
			/sbin/ifconfig $NASName hw ether $VOIP_MAC
			fi
		fi
	fi
}
...
if [ $ISP = "0" ] ; then
    ...
    if [ "$TCSUPPORT_PON_UNIQUE_MAC" != "" ]; then
		/sbin/ifconfig $NASName hw ether $WAN_MAC
    fi
	
	if [ "$TCSUPPORT_HENAN_CABLE" != "" ];then
	echo $ServiceList | grep "TR069"
	if [ $? = 0 ];then
		/sbin/ifconfig $NASName hw ether $WAN_MAC
	fi
	fi
	
	if [ "$TCSUPPORT_EXCITEL" != "" -o "$TCSUPPORT_CUSTOMER_SYROTECH_EXCITEL" != "" ];then
		customized_mac
	fi
	
	if [ -n $TCSUPPORT_PON_FIXED_SEQUENCED_MAC ];then
		if [ -n $WanMac ];then
			/sbin/ifconfig $NASName hw ether $WanMac
		fi
	fi
	...
	if [ "$TCSUPPORT_CT_IPV4_RADIO" != "" ]; then
		if [ "$IPProtocolVersion" = "3" ] ; then
			if [ "$IPVERSION" != "IPv6" ] ; then
				if [ "$DhcpManualDNS" = "1" ]; then
					echo "$FirstDhcpDns" > /var/run/$WAN_IF/dns
					echo "$SecDhcpDns" >> /var/run/$WAN_IF/dns	
					/sbin/udhcpc -i $WAN_IF -s /usr/script/udhcpc_nodns.sh -p /var/run/$WAN_IF/udhcpc.pid &
				else
				...
				if [ "$NATENABLE" = "Enable" ] ; then
					...
					if [ "$TCSUPPORT_NAT_LOOPBACK" != "" ]; then
						iptables -t nat -N $VS_LOOPBACK_CHAIN
					fi
					iptables -t nat -N $DMZ_CHAIN
					iptables -t nat -A PREROUTING -j $VS_CHAIN
					if [ "$TCSUPPORT_NAT_LOOPBACK" != "" ]; then
						iptables -t nat -A PREROUTING -j $VS_LOOPBACK_CHAIN
					fi
...
#	PPP_PARAM="unit $PPPUNIT user $USERNAME password $PASSWORD nodetach holdoff 4 maxfail 0 usepeerdns lcp-echo-interval 1 lcp-echo-failure 60"
    if [ "$TCSUPPORT_CUSTOMER_AIRTEL" != "" ]; then
    PPP_PARAM="nodetach holdoff 4 maxfail 0 usepeerdns lcp-echo-interval 30 lcp-echo-failure 6"
    else
    ...
    elif [ "$TCSUPPORT_CUSTOMER_FUSION" != "" ]; then
			PPP_PARAM="$PPP_PARAM pppoe_srv_name fusionnet"
    elif [ "$TCSUPPORT_CUSTOMER_COMWAY" != "" ]; then
			PPP_PARAM="$PPP_PARAM pppoe_srv_name comway"
    elif [ "$TCSUPPORT_CUSTOMER_RI" != "" ]; then
			PPP_PARAM="$PPP_PARAM pppoe_srv_name TIMBL"
		fi

		if [ "$TCSUPPORT_CUSTOMER_CANDID_NETMAX" != "" ] ; then
			PPP_PARAM="$PPP_PARAM pppoe_ac_name NETMAXINDIA"
		fi
...
#	echo "pppd $PPP_PARAM &" > /var/tmp/$PPP_IF.conf
	...
	if [ "$USERNAME" = "" ]; then
		...
		if [ "$TCSUPPORT_CUSTOMER_FUSION" != "" ]; then
			USERNAME=$USERNAME"@fusionnet.in"
		elif [ "$TCSUPPORT_CUSTOMER_COMWAY" != "" ] || [ "$TCSUPPORT_CUSTOMER_801FUSIONNET" != "" ]; then
			USERNAME=$USERNAME"@comway.in"
		elif [ "$TCSUPPORT_CUSTOMER_RI" != "" ]; then
			USERNAME=$USERNAME"@rinetworks.in"
		elif [ "$TCSUPPORT_UNIWAY_NEXTRA" != "" ]; then
			USERNAME=$USERNAME"@nextraworld.com"
		fi
...
#endif/*TCSUPPORT_COMPILE*/
    if [ "$TCSUPPORT_PON_UNIQUE_MAC" != "" ]; then
        if [ "$VTAG_MODE" = "2" -a "$VTAG_VID" = "4097" ] ; then
            /sbin/ifconfig $NASName hw ether $WAN_BR_MAC
        else
            /sbin/ifconfig $NASName hw ether $WAN_MAC
        fi
    fi
