...
If Request_Form("ACL_Flag") = "1" Then 
	TCWebApi_set("ACL_Common","Activate","ACLActive")
...
function addNewACLRule()
{
	...
	xmlhttp.onreadystatechange=function()
	{
			if (xmlhttp.readyState==4 && xmlhttp.status==200)
			{
				...
				var kIdleEnryIndex = kArrayStr.split(',');
				if (10 < aclNum && 1 == kIdleEnryIndex.length) {
...
function showACL()
{
	var enb="<% tcWebApi_get("ACL_Common","Activate","s") %>";
	if ('No' == enb) {
		showAcls(0);
	} else if ('Yes' == enb) {
		showAcls(1);
              ...
              <TR>
                ...
                <TD>
                  ...
                  <DIV id=secACL>
					...
					<input type="hidden" name="ACLActive" value="<% If TCWebApi_get("ACL_Common","Activate","h") = "Yes" then asp_Write("Yes") else asp_write("No") end if%>">
					<input type="hidden" name="ACLCurIndex" value="<% If TCWebApi_get("ACL_Common","totalNum","h") <> "N/A" then TCWebApi_get("ACL_Common","totalNum","s") else asp_write("0") end if%>">
					<input type="hidden" name="ACLRuleNum" value="99">
					...
					<INPUT TYPE="HIDDEN" NAME="EntrySW"  id="EntrySW" VALUE="No" >
                  <TABLE width=500 cellSpacing=0 cellPadding=0 border=0>
                    <TBODY>
					<div id=keywordEnb width=500>
						<td width="30%" vAlign=middle>Enable ACL:</td>
						<td width="70%"><INPUT id="ACLSwitch" onclick="setACLSwitch(0)" type="checkbox" name="ACLSwitch" <%if tcWebApi_get("ACL_Common","Activate","h") = "Yes" then asp_Write("checked") end if%>></td></div>	
					<tr>
						<td width="30%">&nbsp;</td></tr>
					<tr id=addActive>
						<td width="30%" >Enable:</td>
						<td width="70%" ><INPUT id="ACLEntrySwitch" onclick="setACLSwitch(1)" type="checkbox" name="ACLEntrySwitch"></td></tr>
					<TR id=addInterface>
						<TD width="30%">Interface:</TD><TD colspan='2'>
						<SELECT onchange=InterfaceChange() size=1 name="InterfaceChoose">
						<OPTION selected>Wan</OPTION>
						<OPTION>Lan</OPTION>
						<OPTION>Both</OPTION>
						</SELECT>
						&nbsp;&nbsp;
					...
					<TR id=startIP>
						<TD width="30%">Start IP:</TD><TD colspan='2'><INPUT id=startIPs maxLength=35 size=15 name=startIPs>note:0.0.0.0-0.0.0.0 means all ip</TD></TR>
					<TR id=endIP>
						<TD width="30%">End IP:</TD><TD colspan='2'><INPUT id=endIPs maxLength=35 size=15 name=endIPs></TD></TR>
					<TR id=targetj>
						<TD width="30%">Mode:</TD>
						<TD><SELECT name="ACLAction">
							<OPTION selected>permit</OPTION>
							<OPTION>deny</OPTION></SELECT></TD></TR>
					<tr id=appj>
					...
					//initSetApp();
						...
						<td width="30%">&nbsp;</td></tr>						
					<TR id=acladd>   
						<TD><INPUT onclick="ACLBtnAdd()" type=button value=" Add "></TD></TR>
							

					<tr id=aclUsrInfo><td colspan='2'><iframe src="/cgi-bin/getACLRule.cgi" frameborder="0" width="550"></iframe></td></tr>
								   
					<tr>
						<td width="30%">&nbsp;</td></tr>											
					<tr id=aclDel>
						<TD align=left width=70 colspan='2'><INPUT onclick="ACLBtnDel()" type=button value=" Del "></tr>	
